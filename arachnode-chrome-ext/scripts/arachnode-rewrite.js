...
class ArachNode {
    constructor() {
        this.arachnode_status = "ArachNode Connected!\nWaiting on browser activity.";
        this.arachnode_hierarchy = undefined;
        this.arachnode_on = undefined;
    ...
    addMessageListeners() {
        chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
            if (message?.type == 'arachnode_status_req' && arachnode_status != (message.msg ?? [undefined])[0]) {
                ...
                setTimeout(() => { arachnode_hierarchy = undefined; }, 500);
            } else if (message?.type == 'arachnode_toggle') arachnode_on = message.msg;
        });
    }
}

HTMLElement.prototype.getElementHierarchy = () => {
    var hierarchy = [];
    var last_ancestors = [];
    var relevance_depth = parseInt(document.getElementsByTagName('html')[0].getAttribute('data-arachnode-relevance-depth'));
    $(document).find('*').each(function () {
        if ($.contains(this, this.HTMLElement)) {
        ...
        if ($.contains(this.HTMLElement, this)) hierarchy.push(getAttributes(this, "descendant", -1));
        if ($(this).siblings().filter($(this.HTMLElement)).length > 0) hierarchy.push(getAttributes(this, "sibling", 0));
    });
